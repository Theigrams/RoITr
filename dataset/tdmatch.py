...
from dataset.common import (
    build_ppf_patch,
    calc_gt_patch_correspondence,
    calc_patch_overlap_ratio,
    calc_ppf_cpu,
    collect_local_neighbors,
    farthest_point_subsampling,
    get_square_distance_matrix,
    normal_redirect,
    point2node_sampling,
    sample_gt_node_correspondence,
)
...
class TDMatchDataset(data.Dataset):
    ...
    def __init__(self, infos, config, data_augmentation=True):
        ...
        # view point
        self.view_point = np.array([0.0, 0.0, 0.0])
    ...
    def __getitem__(self, index):
        ...
        if self.data_augmentation:
            ...
            euler_ab = np.random.rand(3) * np.pi * 2.0 / self.rot_factor  # anglez, angley, anglex
        ...
        return (
            src_pcd.astype(np.float32),
            tgt_pcd.astype(np.float32),
            src_normals.astype(np.float32),
            tgt_normals.astype(np.float32),
            src_feats.astype(np.float32),
            tgt_feats.astype(np.float32),
