...
def get_square_distance_matrix(src, tgt):
    ...
    distance = -2.0 * np.matmul(src, tgt.transpose())
...
def get_batched_square_distance_matrix(src, tgt):
    ...
    distance = -2.0 * np.matmul(src, tgt.transpose(0, 2, 1))
    distance += np.sum(src**2, axis=-1, keepdims=False)[:, :, np.newaxis]
    distance += np.sum(tgt**2, axis=-1, keepdims=False)[:, np.newaxis, :]
    distance = np.clip(distance, a_min=1e-12, a_max=None)
...
def calc_gt_patch_correspondence(src_patch, tgt_patch, rot, trans, distance_thres=0.0375):
    ...
    gt_patch_corr[:, :-1, -1] = np.clip(1.0 - gt_patch_corr[:, :-1, :-1].sum(-1), a_min=0.0, a_max=None)
    gt_patch_corr[:, -1, :-1] = np.clip(1.0 - gt_patch_corr[:, :-1, :-1].sum(-2), a_min=0.0, a_max=None)
...
def calc_ppf_cpu(point1, normal1, point2, normal2, neighbors):
    ...
    # angle(n1, d)
    y = np.sum(
        normal1[:, np.newaxis, :].repeat(patch_normals.shape[1], axis=1) * d, axis=-1, keepdims=True
    )  # [n, num_point_per_patch, ]
    x = np.linalg.norm(
        np.cross(normal1[:, np.newaxis, :].repeat(patch_normals.shape[1], axis=1), d), axis=-1, keepdims=True
    )
    ...
    # angle(n1, n2)
    y = np.sum(
        normal1[:, np.newaxis, :].repeat(patch_normals.shape[1], axis=1) * patch_normals, axis=-1, keepdims=True
    )  # [n, num_point_per_patch, ]
    x = np.linalg.norm(
        np.cross(normal1[:, np.newaxis, :].repeat(patch_normals.shape[1], axis=1), patch_normals),
        axis=-1,
        keepdims=True,
    )
...
def calc_patch_overlap_ratio(
    src_nodes, src_patches, src_masks, tgt_nodes, tgt_patches, tgt_masks, rot, trans, overlap_control=0.0375
):
    """
    ...
    transformed_src_patches = (
        np.matmul(rot[np.newaxis, :], src_patches.transpose(0, 2, 1)) + trans[np.newaxis, :]
    ).transpose(
        0, 2, 1
    )  # [M1, K, 3]
    ...
    patchwise_mask = (
        np.sqrt(get_batched_square_distance_matrix(sel_src_patches, sel_tgt_patches)) < overlap_control
    )  # [X, K, K]
    ...
    row_major_overlap_ratio_matrix[
        row_major_possible_corr[0], row_major_possible_corr[1]
    ] = row_major_overlap_ratio  # [M1, M2]
    ...
    col_major_overlap_ratio_matrix[
        col_major_possible_corr[0], col_major_possible_corr[1]
    ] = col_major_overlap_ratio  # [M1, M2]
...
def x_axis_crop(pcd, min_overlap=0.3):
    ...
    left_max = np.random.uniform(0.6, 1.0)
    right_min = np.random.uniform(0.0, 0.4)
    left_min = 0.0
    right_max = 1.0
