    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)):
                ...
                data["src_node_desc"], data["tgt_node_desc"] = (
                    outputs["src_node_feats"].cpu().detach(),
                    outputs["tgt_node_feats"].cpu().detach(),
                )
                data["src_point_desc"], data["tgt_point_desc"] = (
                    outputs["src_point_feats"].cpu().detach(),
                    outputs["tgt_point_feats"].cpu().detach(),
                )
                data["src_corr_pts"], data["tgt_corr_pts"] = (
                    outputs["src_corr_points"].cpu(),
                    outputs["tgt_corr_points"].cpu(),
                )
