...
from model.modules import (
    AdaptiveSuperPointMatching,
    CoarseMatching,
    FineMatching,
    GTCoarseCorrGenerator,
...
class RIGA_v2(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.benchmark in ["3DMatch", "3DLoMatch"]:
            self.coarse_matching = CoarseMatching(
                num_correspondences=config.num_est_coarse_corr, dual_normalization=True
            )
            self.factor = 1
        else:
            self.coarse_matching = AdaptiveSuperPointMatching(
                min_num_correspondences=config.num_est_coarse_corr, similarity_threshold=0.75
            )
            self.factor = 2

        self.backbone = RIPointTransformer(
            transformer_architecture=config.transformer_architecture,
            with_cross_pos_embed=self.with_cross_pos_embed,
            factor=self.factor,
        )
        ...
        self.coarse_generator = GTCoarseCorrGenerator(
            num_targets=config.num_gt_coarse_corr, overlap_threshold=config.coarse_overlap_threshold
        )
        self.fine_matching = FineMatching(
            config.fine_matching_topk,
            mutual=config.fine_matching_mutual,
            confidence_threshold=config.fine_matching_confidence_threshold,
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, src_normals, tgt_normals, rot, trans, src_raw_pcd):
        src_o, tgt_o = (
            torch.from_numpy(np.array([src_raw_pcd.shape[0]])).to(src_raw_pcd).int(),
            torch.from_numpy(np.array([tgt_pcd.shape[0]])).to(tgt_pcd).int(),
        )
        output_dict = {}
        # 1. get descriptors
        (
            ...
            [src_raw_pcd, src_feats, src_o, src_normals], [tgt_pcd, tgt_feats, tgt_o, tgt_normals], src_pcd
        )
        ...
        _, src_node_masks, src_node_knn_indices, src_node_knn_masks = point_to_node_partition(
            src_pcd, src_node_xyz, point_limit=self.point_per_patch
        )
        _, tgt_node_masks, tgt_node_knn_indices, tgt_node_knn_masks = point_to_node_partition(
            tgt_pcd, tgt_node_xyz, point_limit=self.point_per_patch
        )
        ...
        gt_tgt_node_occ_score, gt_src_node_occ_score = get_node_occlusion_score(
        ...
        with torch.no_grad():
            tgt_node_corr_indices, src_node_corr_indices, node_corr_scores = self.coarse_matching(
            ...
            if self.training:
                tgt_node_corr_indices, src_node_corr_indices, node_corr_scores = self.coarse_generator(
                    gt_node_corr_indices, gt_node_corr_overlaps.contiguous()
                )
        ...
        # 5. Optimal transport
        matching_scores = torch.einsum("bnd,bmd->bnm", tgt_node_corr_knn_feats, src_node_corr_knn_feats)  # (P, K, K)
