...
class RIPointTransformerLayer(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, num_heads=4, nsample=16, factor=1):
        ...
        self.nsample = nsample
        self.factor = factor
        self.transformer = LocalPPFTransformer(
            input_dim=in_planes, hidden_dim=min(out_planes, 256 * factor), output_dim=out_planes, num_heads=num_heads
        )
...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxon):
        ...
        group_idx = pointops.queryandgroup(
            self.nsample, p, n_p, p, None, o, n_o, return_idx=True
        ).long()  # (m, nsample, 3 + 4 + c)
    ...
    def __init__(self, in_planes, out_planes=None):
        ...
        if out_planes is None:
            self.linear1 = nn.Sequential(
                nn.Linear(2 * in_planes, in_planes), nn.LayerNorm(in_planes), nn.ReLU(inplace=True)
            )
            self.linear2 = nn.Sequential(nn.Linear(in_planes, in_planes), nn.ReLU(inplace=True))
        else:
            self.linear1 = nn.Sequential(
                nn.Linear(out_planes, out_planes), nn.LayerNorm(out_planes), nn.ReLU(inplace=True)
            )
            self.linear2 = nn.Sequential(
                nn.Linear(in_planes, out_planes), nn.LayerNorm(out_planes), nn.ReLU(inplace=True)
            )
...
class RIPointTransformer(nn.Module):
    def __init__(
        self,
        blocks=[2, 3, 3, 3],
        block=RIPointTransformerBlock,
        c=1,
        transformer_architecture=None,
        with_cross_pos_embed=None,
        factor=1,
        occ_thres=0.0,
    ):
        super().__init__()
        ...
        self.enc1 = self._make_enc(
            block, planes[0], blocks[0], self.num_heads, stride=stride[0], nsample=nsample[0], factor=factor
        )  # N/1

        self.enc2 = self._make_enc(
            block, planes[1], blocks[1], self.num_heads, stride=stride[1], nsample=nsample[1], factor=factor
        )  # N/4
        self.enc3 = self._make_enc(
            block, planes[2], blocks[2], self.num_heads, stride=stride[2], nsample=nsample[2], factor=factor
        )  # N/16
        self.enc4 = self._make_enc(
            block, planes[3], blocks[3], self.num_heads, stride=stride[3], nsample=nsample[3], factor=factor
        )  # N/64

        self.dec4 = self._make_dec(
            block, planes[3], 2, self.num_heads, nsample=nsample[3], factor=factor, is_head=True
        )  # fusion p5 and p4
        self.dec3 = self._make_dec(
            block, planes[2], 2, self.num_heads, nsample=nsample[2], factor=factor
        )  # fusion p4 and p3
        self.dec2 = self._make_dec(
            block, planes[1], 2, self.num_heads, nsample=nsample[1], factor=factor
        )  # fusion p3 and p2
        self.dec1 = self._make_dec(
            block, planes[0], 2, self.num_heads, nsample=nsample[0], factor=factor
        )  # fusion p2 and p1
        self.nsample = nsample
        self.transformer_architecture = transformer_architecture

        self.global_transformer = GeometricTransformer(
            256 * factor,
            256 * factor,
            256 * factor,
            4,
        ...
        self.with_cross_pos_embed = (
            with_cross_pos_embed  # whether to use the learned rotation-invariant cross-frame positional representation
        )
    ...
    def forward(self, s_pxon, t_pxon, src_deformed_pcd):
        ...
        s_g_x4, t_g_x4 = self.global_transformer(
            s_p4.unsqueeze(0), t_p4.unsqueeze(0), s_x4.unsqueeze(0), t_x4.unsqueeze(0)
        )
        s_g_x4 = s_g_x4[0]
        t_g_x4 = t_g_x4[0]
        ...
        s_x3 = self.dec3[1:](
            [s_p3, self.dec3[0]([s_p3, s_x3, s_o3], [s_p4, s_x4, s_o4]), s_o3, s_n3, s_idx3, s_ppf3, None]
        )[1]
        s_x2 = self.dec2[1:](
            [s_p2, self.dec2[0]([s_p2, s_x2, s_o2], [s_p3, s_x3, s_o3]), s_o2, s_n2, s_idx2, s_ppf2, None]
        )[1]
        s_x1 = self.dec1[1:](
            [s_p1, self.dec1[0]([s_p1, s_x1, s_o1], [s_p2, s_x2, s_o2]), s_o1, s_n1, s_idx1, s_ppf1, None]
        )[1]
        # target pcd
        t_x4 = self.dec4[1:]([t_p4, self.dec4[0]([t_p4, t_x4, t_o4]), t_o4, t_n4, t_idx4, t_ppf4, None])[1]
        t_x3 = self.dec3[1:](
            [t_p3, self.dec3[0]([t_p3, t_x3, t_o3], [t_p4, t_x4, t_o4]), t_o3, t_n3, t_idx3, t_ppf3, None]
        )[1]
        t_x2 = self.dec2[1:](
            [t_p2, self.dec2[0]([t_p2, t_x2, t_o2], [t_p3, t_x3, t_o3]), t_o2, t_n2, t_idx2, t_ppf2, None]
        )[1]
        t_x1 = self.dec1[1:](
            [t_p1, self.dec1[0]([t_p1, t_x1, t_o1], [t_p2, t_x2, t_o2]), t_o1, t_n1, t_idx1, t_ppf1, None]
        )[1]
