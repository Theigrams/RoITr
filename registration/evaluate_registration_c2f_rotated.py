...
from lib.utils import natural_key, setup_seed, square_distance, weighted_procrustes
from registration.benchmark import benchmark
from registration.benchmark_utils import (
    get_inlier_ratio_correspondence,
    get_scene_split,
    ransac_pose_estimation_correspondences,
...
def benchmark_registration(
    desc, exp_dir, whichbenchmark, n_points, ransac_with_mutual=False, inlier_ratio_threshold=0.05
):
    ...
    for eachfile in tqdm(desc):
        ...
        est_transformed_pcd = (
            torch.matmul(src_pcd, torch.from_numpy(est_rot.T).float()) + torch.from_numpy(est_trans.T).float()
        )
        ...
        cur_inlier_ratio = get_inlier_ratio_correspondence(
            src_corr_pts, tgt_corr_pts, rot, trans, inlier_distance_threshold=0.1
        )
