...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, return_idx=False, use_xyz=True):
    """
    ...
    if new_xyz is None:
        new_xyz = xyz
    if idx is None:
        idx, _ = knnquery(nsample + 1, xyz, new_xyz, offset, new_offset)  # (m, nsample)
        idx = idx[:, 1:].contiguous().long()
...
class Aggregation(Function):
    ...
    def backward(ctx, grad_output):
        ...
        pointops_cuda.aggregation_backward_cuda(
            n, nsample, c, w_c, input, position, weight, idx, grad_output, grad_input, grad_position, grad_weight
        )
