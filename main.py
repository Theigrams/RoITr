...
from dataset.dataloader import get_dataloader, get_dataset
from lib.loss import Evaluator, OverallLoss
...
def main():
    ...
    if config.local_rank >= 0:
        config.model = torch.nn.parallel.DistributedDataParallel(
            config.model, device_ids=[args.local_rank], output_device=args.local_rank, find_unused_parameters=True
        )
    ...
    if config.local_rank > -1:
        train_sampler, val_sampler, benchmark_sampler = (
            DistributedSampler(train_set),
            DistributedSampler(val_set),
            DistributedSampler(benchmark_set),
        )
    else:
    ...
    config.val_loader = get_dataloader(
        val_set, sampler=val_sampler, batch_size=1, num_workers=config.num_workers, shuffle=False, drop_last=False
    )
    config.test_loader = get_dataloader(
        benchmark_set,
